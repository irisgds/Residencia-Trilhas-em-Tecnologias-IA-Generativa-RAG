...
O desenho abaixo reúne **pré-produção** e **produção** em um único fluxo, evitando repetir o mesmo pipeline em três diagramas.
...
O desenho reúne a ingestão multimodal, a recuperação por RAG e o encaminhamento de perguntas estruturadas para SQL/API.

```mermaid
flowchart TD
    A[Laudos, relatórios, manuais, imagens] --> B[Extração / OCR]
    B --> C[Tratamento de tabelas e imagens]
    C --> D[Limpeza e normalização]
    D --> E[Metadados]
    E --> F[Chunking por seção / parágrafo]
    F --> G[Embeddings BGE-M3]
    G --> H[(Banco vetorial)]

    I[Pergunta do técnico] --> J{Pergunta estruturada?}
    J -->|Não| K[Embedding / reformulação]
    K --> L[Retriever]
    H --> L
    L --> M[Busca semântica + filtros]
    M --> N[Top-K / reranking opcional]
    N --> O[Contexto]
    O --> P[Prompt]
    P --> Q[LLM]
    Q --> R[Resposta + fontes]

    J -->|Sim| S[Banco de serviços / SQL / API]
    S --> R
```
